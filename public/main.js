...
getHTML.onclick=()=>{
    ...
    request.onload=()=>{
        ...
        div.innerHTML=request.response//填写div的内容
        document.body.appendChild(div)//将这个div插入到body里面
    }
    request.onerror=()=>{}
    request.send()
};
getJS.onclick=()=>{
    ...
    request.onload=()=>{
        ...
        document.body.appendChild(script)//将script插入到body里面    
    }
    request.onerror=()=>{}
    request.send()
}
...
request.onreadystatechange=()=>{
    if(request.readyState===4){//下载完成，但不知道是成功2xx,还是失败4xx,5xx
